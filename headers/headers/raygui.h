...
#define GUI_SCROLL_DELTA                                                                                                                             \
    ((float)IsGamepadButtonDown (0, GAMEPAD_BUTTON_RIGHT_TRIGGER_2) - (float)IsGamepadButtonDown (0, GAMEPAD_BUTTON_LEFT_TRIGGER_2))
#else
#define GUI_SCROLL_DELTA                                                                                                                             \
...
{ // Prevents name mangling of functions
    ...
    RAYGUIAPI int GuiProgressBar (Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue,
...
#define CHECK_BOUNDS_ID(src, dst)                                                                                                                    \
    (((int)src.x == (int)dst.x) && ((int)src.y == (int)dst.y) && ((int)src.width == (int)dst.width) && ((int)src.height == (int)dst.height))
#endif
...
#define RAYGUI_WINDOWBOX_CLOSEBUTTON_HEIGHT 18
    ...
    Rectangle windowPanel    = { bounds.x, bounds.y + (float)statusBarHeight - (float)statusBorderWidth, bounds.width,
...
#define RAYGUI_GROUPBOX_LINE_THICK 1
    ...
    GuiDrawRectangle (RAYGUI_CLITERAL (Rectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, BLANK,
                      GetColor (GuiGetStyle (DEFAULT, (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
    GuiDrawRectangle (RAYGUI_CLITERAL (Rectangle){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK,
                      GetColor (GuiGetStyle (DEFAULT, (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
...
#define RAYGUI_LINE_TEXT_PADDING 4
    ...
    if (text == NULL) GuiDrawRectangle (RAYGUI_CLITERAL (Rectangle){ bounds.x, bounds.y + bounds.height / 2, bounds.width, 1 }, 0, BLANK, color);
    else
        {
            ...
            GuiDrawRectangle (
                RAYGUI_CLITERAL (Rectangle){ bounds.x, bounds.y + bounds.height / 2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0,
                BLANK, color);
...
int
GuiTabBar (Rectangle bounds, char** text, int count, int* active)
{
...
#define RAYGUI_MIN_MOUSE_WHEEL_SPEED 20
    ...
    if (!hasHorizontalScrollBar)
        hasHorizontalScrollBar
            = (hasVerticalScrollBar
               && (content.width > (bounds.width - 2 * GuiGetStyle (DEFAULT, BORDER_WIDTH) - GuiGetStyle (LISTVIEW, SCROLLBAR_WIDTH))))
                  ? true
                  : false;
    if (!hasVerticalScrollBar)
        hasVerticalScrollBar
            = (hasHorizontalScrollBar
               && (content.height > (bounds.height - 2 * GuiGetStyle (DEFAULT, BORDER_WIDTH) - GuiGetStyle (LISTVIEW, SCROLLBAR_WIDTH))))
                  ? true
                  : false;
    ...
    Rectangle horizontalScrollBar
        = { (float)((GuiGetStyle (LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ? (float)bounds.x + verticalScrollBarWidth : (float)bounds.x)
                + GuiGetStyle (DEFAULT, BORDER_WIDTH),
            (float)bounds.y + bounds.height - horizontalScrollBarWidth - GuiGetStyle (DEFAULT, BORDER_WIDTH),
            (float)bounds.width - verticalScrollBarWidth - 2 * GuiGetStyle (DEFAULT, BORDER_WIDTH), (float)horizontalScrollBarWidth };
    Rectangle verticalScrollBar = { (float)((GuiGetStyle (LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
    ...
    *view = (GuiGetStyle (LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
                ...
                : RAYGUI_CLITERAL (Rectangle){ bounds.x + GuiGetStyle (DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle (DEFAULT, BORDER_WIDTH),
                                               bounds.width - 2 * GuiGetStyle (DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth,
                                               bounds.height - 2 * GuiGetStyle (DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth };
    ...
    float horizontalMin = hasHorizontalScrollBar
                              ...
                              : (((float)GuiGetStyle (LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ? (float)-verticalScrollBarWidth : 0)
                                    - (float)GuiGetStyle (DEFAULT, BORDER_WIDTH);
    ...
    float verticalMax   = hasVerticalScrollBar
                              ? content.height - bounds.height + (float)horizontalScrollBarWidth + (float)GuiGetStyle (DEFAULT, BORDER_WIDTH)
                              : (float)-GuiGetStyle (DEFAULT, BORDER_WIDTH);
    ...
    if (hasVerticalScrollBar)
        {
            ...
            GuiSetStyle (SCROLLBAR, SCROLL_SLIDER_SIZE,
                         (int)(((bounds.height - 2 * GuiGetStyle (DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth) / (int)content.height)
    ...
    if (hasHorizontalScrollBar && hasVerticalScrollBar)
        {
            Rectangle corner
                = { (GuiGetStyle (LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ? (bounds.x + GuiGetStyle (DEFAULT, BORDER_WIDTH) + 2)
                                                                                    : (horizontalScrollBar.x + horizontalScrollBar.width + 2),
                    verticalScrollBar.y + verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4, (float)verticalScrollBarWidth - 4 };
...
GuiCheckBox (Rectangle bounds, const char* text, bool* checked)
{
    ...
    GuiDrawText (text, textBounds, (GuiGetStyle (CHECKBOX, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT) ? TEXT_ALIGN_LEFT : TEXT_ALIGN_RIGHT,
...
GuiDropdownBox (Rectangle bounds, const char* text, int* active, bool editMode)
{
    ...
    if (direction == 1)
        boundsOpen.y
            -= itemCount * (bounds.height + GuiGetStyle (DROPDOWNBOX, DROPDOWN_ITEMS_SPACING)) + GuiGetStyle (DROPDOWNBOX, DROPDOWN_ITEMS_SPACING);
    ...
    GuiDrawRectangle (bounds, GuiGetStyle (DROPDOWNBOX, BORDER_WIDTH), GetColor (GuiGetStyle (DROPDOWNBOX, BORDER + state * 3)),
...
#define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY 1 // Frames delay for autocursor movement
    ...
    Rectangle cursor = { textBounds.x + textWidth + GuiGetStyle (DEFAULT, TEXT_SPACING),
                         textBounds.y + textBounds.height / 2 - GuiGetStyle (DEFAULT, TEXT_SIZE), 2, (float)GuiGetStyle (DEFAULT, TEXT_SIZE) * 2 };
            ...
            if (editMode)
                {
                    ...
                    if (GUI_KEY_DOWN (KEY_LEFT) || GUI_KEY_DOWN (KEY_RIGHT) || GUI_KEY_DOWN (KEY_UP) || GUI_KEY_DOWN (KEY_DOWN)
                        || GUI_KEY_DOWN (KEY_BACKSPACE) || GUI_KEY_DOWN (KEY_DELETE))
                        autoCursorCounter++;
                    else autoCursorCounter = 0;

                    bool autoCursorShouldTrigger
                        = (autoCursorCounter > RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN) && ((autoCursorCounter % RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0);
                    ...
                    if ((textLength > textBoxCursorIndex) && GUI_KEY_PRESSED (KEY_DELETE)
                        && (GUI_KEY_DOWN (KEY_LEFT_CONTROL) || GUI_KEY_DOWN (KEY_RIGHT_CONTROL)))
                        {
                            ...
                            textLength -= accCodepointSize;
                             ...
                             && (GUI_KEY_PRESSED (KEY_DELETE) || (GUI_KEY_DOWN (KEY_DELETE) && autoCursorShouldTrigger)))
                        {
                    ...
                    if ((textBoxCursorIndex > 0) && GUI_KEY_PRESSED (KEY_BACKSPACE)
                        && (GUI_KEY_DOWN (KEY_LEFT_CONTROL) || GUI_KEY_DOWN (KEY_RIGHT_CONTROL)))
                        {
                            ...
                            textBoxCursorIndex -= accCodepointSize;
                             ...
                             && (GUI_KEY_PRESSED (KEY_BACKSPACE) || (GUI_KEY_DOWN (KEY_BACKSPACE) && autoCursorShouldTrigger)))
                        {
                    ...
                    if ((textBoxCursorIndex > 0) && GUI_KEY_PRESSED (KEY_LEFT)
                             ...
                             && (GUI_KEY_DOWN (KEY_LEFT_CONTROL) || GUI_KEY_DOWN (KEY_RIGHT_CONTROL)))
                        {
                            ...
                            textBoxCursorIndex = offset;
                             ...
                             && (GUI_KEY_PRESSED (KEY_RIGHT) || (GUI_KEY_DOWN (KEY_RIGHT) && autoCursorShouldTrigger)))
                        {
                            int nextCodepointSize = 0;
                            GetCodepointNext (text + textBoxCursorIndex, &nextCodepointSize);
                    ...
                    cursor.x = bounds.x + GuiGetStyle (TEXTBOX, TEXT_PADDING) + GuiGetTextWidth (text + textIndexOffset)
                               - GuiGetTextWidth (text + textBoxCursorIndex) + GuiGetStyle (DEFAULT, TEXT_SPACING);
                    // if (multiline) cursor.y = GetTextLines()

                    // Finish text editing on ENTER or mouse click outside bounds
                    if ((!multiline && GUI_KEY_PRESSED (KEY_ENTER)) || (!CheckCollisionPointRec (mousePosition, bounds) && GUI_BUTTON_PRESSED))
                        {
    ...
    if (state == STATE_PRESSED)
        {
            GuiDrawRectangle (bounds, GuiGetStyle (TEXTBOX, BORDER_WIDTH), GetColor (GuiGetStyle (TEXTBOX, BORDER + (state * 3))),
...
int
GuiSpinner (Rectangle bounds, const char* text, int* value, int minValue, int maxValue, bool editMode)
{
    int result     = 1;
    GuiState state = guiState;
    ...
    Rectangle valueBoxBounds
        = { bounds.x + GuiGetStyle (VALUEBOX, SPINNER_BUTTON_WIDTH) + GuiGetStyle (VALUEBOX, SPINNER_BUTTON_SPACING), bounds.y,
    ...
    if (editMode)
        {
            ...
            Rectangle cursor = { bounds.x + GuiGetTextWidth (textValue) / 2 + bounds.width / 2 + 1,
                                 bounds.y + GuiGetStyle (TEXTBOX, BORDER_WIDTH) + 2, 2, bounds.height - GuiGetStyle (TEXTBOX, BORDER_WIDTH) * 2 - 4 };
            if (cursor.height > bounds.height) cursor.height = bounds.height - GuiGetStyle (TEXTBOX, BORDER_WIDTH) * 2;
...
// NOTE: Requires static variables: frameCounter
int
GuiValueBoxFloat (Rectangle bounds, const char* text, char* textValue, float* value, bool editMode)
{
#if !defined(RAYGUI_VALUEBOX_MAX_CHARS)
#define RAYGUI_VALUEBOX_MAX_CHARS 32
#endif
                    ...
                    if (keyCount < RAYGUI_VALUEBOX_MAX_CHARS)
                        {
                            if (GuiGetTextWidth (textValue) < bounds.width)
                                {
                                    int key = GUI_INPUT_KEY;
                                    if (((key >= 48) && (key <= 57)) || (key == '.') || ((keyCount == 0) && (key == '+'))
    ...
    if (editMode)
        {
            // NOTE: ValueBox internal text is always centered
            Rectangle cursor = { bounds.x + GuiGetTextWidth (textValue) / 2 + bounds.width / 2 + 1,
                                 bounds.y + 2 * GuiGetStyle (VALUEBOX, BORDER_WIDTH), 4, bounds.height - 4 * GuiGetStyle (VALUEBOX, BORDER_WIDTH) };
            GuiDrawRectangle (cursor, 0, BLANK, GetColor (GuiGetStyle (VALUEBOX, BORDER_COLOR_PRESSED)));
        }
                            ...
                            if (CHECK_BOUNDS_ID (bounds, guiControlExclusiveRec))
                                {
                                    ...
                                    *value = (maxValue - minValue) * ((mousePoint.x - bounds.x - sliderWidth / 2) / (bounds.width - sliderWidth))
                                             + minValue;
                                }
                        }
                    else
...
int
GuiSliderBar (Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue, float maxValue)
{
    int result         = 0;
...
int
GuiProgressBar (Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue, float maxValue)
{
    int result     = 0;
    GuiState state = guiState;
            ...
            if (*value > minValue)
                {
                    ...
                    GuiDrawRectangle (
                        RAYGUI_CLITERAL (Rectangle){ bounds.x, bounds.y + 1, (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0,
                        BLANK, GetColor (GuiGetStyle (PROGRESSBAR, BORDER_COLOR_FOCUSED)));
                ...
                GuiDrawRectangle (RAYGUI_CLITERAL (Rectangle){ bounds.x, bounds.y, (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH),
                                                               bounds.height + GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) - 1 },
                                  0, BLANK, GetColor (GuiGetStyle (PROGRESSBAR, BORDER_COLOR_NORMAL)));

            if (*value >= maxValue)
                GuiDrawRectangle (RAYGUI_CLITERAL (Rectangle){ bounds.x + progress.width + (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH), bounds.y,
                                                               (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH),
                                                               bounds.height + GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) - 1 },
                                  0, BLANK, GetColor (GuiGetStyle (PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            else
                {
                    ...
                    GuiDrawRectangle (
                        RAYGUI_CLITERAL (Rectangle){ bounds.x + (int)progress.width + (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH), bounds.y,
                                                     bounds.width - (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) - (int)progress.width - 1,
                                                     (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) },
                        0, BLANK, GetColor (GuiGetStyle (PROGRESSBAR, BORDER_COLOR_NORMAL)));
                    GuiDrawRectangle (
                        RAYGUI_CLITERAL (Rectangle){ bounds.x + (int)progress.width + (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH),
                                                     bounds.y + bounds.height - 1,
                                                     bounds.width - (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) - (int)progress.width - 1,
                                                     (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) },
                        0, BLANK, GetColor (GuiGetStyle (PROGRESSBAR, BORDER_COLOR_NORMAL)));
                    GuiDrawRectangle (RAYGUI_CLITERAL (Rectangle){ bounds.x + bounds.width - (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH), bounds.y,
                                                                   (float)GuiGetStyle (PROGRESSBAR, BORDER_WIDTH),
                                                                   bounds.height + GuiGetStyle (PROGRESSBAR, BORDER_WIDTH) - 1 },
                                      0, BLANK, GetColor (GuiGetStyle (PROGRESSBAR, BORDER_COLOR_NORMAL)));
                }
...
GuiStatusBar (Rectangle bounds, const char* text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle (bounds, GuiGetStyle (STATUSBAR, BORDER_WIDTH), GetColor (GuiGetStyle (STATUSBAR, BORDER + (state * 3))),
...
int
GuiListViewEx (Rectangle bounds, char** text, int count, int* scrollIndex, int* active, int* focus)
{
    int result       = 0;
    GuiState state   = guiState;
    ...
    for (int i = 0; ((i < visibleItems) && (text != NULL)); i++)
        {
            if (GuiGetStyle (LISTVIEW, LIST_ITEMS_BORDER_NORMAL))
                GuiDrawRectangle (itemBounds, GuiGetStyle (LISTVIEW, LIST_ITEMS_BORDER_WIDTH), GetColor (GuiGetStyle (LISTVIEW, BORDER_COLOR_NORMAL)),
                                  BLANK);
            ...
            if (state == STATE_DISABLED)
                {
                    ...
                    GuiDrawText (text[startIndex + i], GetTextBounds (LISTVIEW, itemBounds), GuiGetStyle (LISTVIEW, TEXT_ALIGNMENT),
    ...
    if (useScrollBar)
        {
            Rectangle scrollBarBounds = { bounds.x + bounds.width - GuiGetStyle (LISTVIEW, BORDER_WIDTH) - GuiGetStyle (LISTVIEW, SCROLLBAR_WIDTH),
    ...
    if (hsv.x != prevHsv.x || hsv.y != prevHsv.y || hsv.z != prevHsv.z)
        {
            ...
            *color = RAYGUI_CLITERAL (Color){ (unsigned char)(255.0f * rgb.x), (unsigned char)(255.0f * rgb.y), (unsigned char)(255.0f * rgb.z),
                                              color->a };
        }
    return result;
}
...
#define RAYGUI_COLORBARALPHA_CHECKED_SIZE 10
    ...
    Rectangle selector
        = { (float)bounds.x + (*alpha) * bounds.width - GuiGetStyle (COLORPICKER, HUEBAR_SELECTOR_HEIGHT) / 2,
            (float)bounds.y - GuiGetStyle (COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle (COLORPICKER, HUEBAR_SELECTOR_HEIGHT),
            ...
            for (int x = 0; x < checksX; x++)
                {
                    for (int y = 0; y < checksY; y++)
                        {
                            Rectangle check = { bounds.x + x * RAYGUI_COLORBARALPHA_CHECKED_SIZE, bounds.y + y * RAYGUI_COLORBARALPHA_CHECKED_SIZE,
...
GuiColorBarHue (Rectangle bounds, const char* text, float* hue)
{
    ...
    Rectangle selector
        = { (float)bounds.x - GuiGetStyle (COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
            ...
            (float)bounds.width + GuiGetStyle (COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) * 2, (float)GuiGetStyle (COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
    ...
    if (state != STATE_DISABLED)
        {
            ...
            DrawRectangleGradientV ((int)bounds.x, (int)(bounds.y + bounds.height / 6), (int)bounds.width, (int)ceilf (bounds.height / 6),
                                    Fade (RAYGUI_CLITERAL (Color){ 255, 255, 0, 255 }, guiAlpha),
                                    Fade (RAYGUI_CLITERAL (Color){ 0, 255, 0, 255 }, guiAlpha));
            DrawRectangleGradientV ((int)bounds.x, (int)(bounds.y + 2 * (bounds.height / 6)), (int)bounds.width, (int)ceilf (bounds.height / 6),
                                    Fade (RAYGUI_CLITERAL (Color){ 0, 255, 0, 255 }, guiAlpha),
                                    Fade (RAYGUI_CLITERAL (Color){ 0, 255, 255, 255 }, guiAlpha));
            DrawRectangleGradientV ((int)bounds.x, (int)(bounds.y + 3 * (bounds.height / 6)), (int)bounds.width, (int)ceilf (bounds.height / 6),
                                    Fade (RAYGUI_CLITERAL (Color){ 0, 255, 255, 255 }, guiAlpha),
                                    Fade (RAYGUI_CLITERAL (Color){ 0, 0, 255, 255 }, guiAlpha));
            DrawRectangleGradientV ((int)bounds.x, (int)(bounds.y + 4 * (bounds.height / 6)), (int)bounds.width, (int)ceilf (bounds.height / 6),
                                    Fade (RAYGUI_CLITERAL (Color){ 0, 0, 255, 255 }, guiAlpha),
                                    Fade (RAYGUI_CLITERAL (Color){ 255, 0, 255, 255 }, guiAlpha));
            DrawRectangleGradientV ((int)bounds.x, (int)(bounds.y + 5 * (bounds.height / 6)), (int)bounds.width, (int)(bounds.height / 6),
                                    Fade (RAYGUI_CLITERAL (Color){ 255, 0, 255, 255 }, guiAlpha),
                                    Fade (RAYGUI_CLITERAL (Color){ 255, 0, 0, 255 }, guiAlpha));
        }
    else
...
GuiColorPicker (Rectangle bounds, const char* text, Color* color)
{
    ...
    // Rectangle boundsAlpha = { bounds.x, bounds.y + bounds.height + GuiGetStyle(COLORPICKER, BARS_PADDING), bounds.width, GuiGetStyle(COLORPICKER,
    // BARS_THICK) };

    // NOTE: this conversion can cause low hue-resolution, if the r, g and b value are very similar, which causes the hue bar to shift around when
...
// Message Box control
int
GuiMessageBox (Rectangle bounds, const char* title, const char* message, const char* buttons)
{
...
int
GuiTextInputBox (Rectangle bounds, const char* title, const char* message, const char* buttons, char* text, int textMaxSize, bool* secretViewActive)
{
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT 26
    ...
    if (secretViewActive != NULL)
        {
            static char stars[] = "****************";
            if (GuiTextBox (RAYGUI_CLITERAL (Rectangle){ textBoxBounds.x, textBoxBounds.y, textBoxBounds.width - 4 - RAYGUI_TEXTINPUTBOX_HEIGHT,
                                                         textBoxBounds.height },
                            ((*secretViewActive == 1) || textEditMode) ? text : stars, textMaxSize, textEditMode))
                textEditMode = !textEditMode;

            GuiToggle (RAYGUI_CLITERAL (Rectangle){ textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.y,
    ...
    if (guiFont.texture.id != GetFontDefault ().texture.id)
        {
            ...
            SetShapesTexture (guiFont.texture,
                              RAYGUI_CLITERAL (Rectangle){ whiteChar.x + 1, whiteChar.y + 1, whiteChar.width - 2, whiteChar.height - 2 });
        }
}
                            ...
                            if ((recsDataCompressedSize > 0) && (recsDataCompressedSize != recsDataSize))
                                {
                                    ...
                                    unsigned char* recsDataCompressed
                                        = (unsigned char*)RAYGUI_CALLOC (recsDataCompressedSize, sizeof (unsigned char));
                            ...
                            if ((glyphsDataCompressedSize > 0) && (glyphsDataCompressedSize != glyphsDataSize))
                                {
                                    ...
                                    unsigned char* glypsDataCompressed
                                        = (unsigned char*)RAYGUI_CALLOC (glyphsDataCompressedSize, sizeof (unsigned char));
...
GetTextBounds (int control, Rectangle bounds)
{
    ...
    textBounds.height
        = bounds.height - 2 * GuiGetStyle (control, BORDER_WIDTH) - 2 * GuiGetStyle (control, TEXT_PADDING); // NOTE: Text is processed line per line!
    ...
    for (int i = 0; i < lineCount; i++)
        {
            ...
            else if ((wrapMode == TEXT_WRAP_CHAR) || (wrapMode == TEXT_WRAP_WORD)) posOffsetY += (textOffsetY + GuiGetStyle (DEFAULT, TEXT_SIZE));
            //---------------------------------------------------------------------------------
        }
    ...
    if (borderWidth > 0)
        {
            ...
            DrawRectangle ((int)rec.x + (int)rec.width - borderWidth, (int)rec.y + borderWidth, borderWidth, (int)rec.height - 2 * borderWidth,
...
GuiTooltip (Rectangle controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiControlExclusiveMode)
        {
            Vector2 textSize
                = MeasureTextEx (GuiGetFont (), guiTooltipPtr, (float)GuiGetStyle (DEFAULT, TEXT_SIZE), (float)GuiGetStyle (DEFAULT, TEXT_SPACING));
            ...
            GuiPanel (
                RAYGUI_CLITERAL (Rectangle){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, textSize.y + 8.0f * lineCount },
                NULL);
...
GuiScrollBar (Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    const int spinnerSize = GuiGetStyle (SCROLLBAR, ARROWS_VISIBLE) ? (isVertical ? (int)bounds.width - 2 * GuiGetStyle (SCROLLBAR, BORDER_WIDTH)
    ...
    if (isVertical)
        {
            ...
            scrollbar      = RAYGUI_CLITERAL (Rectangle){
                bounds.x + GuiGetStyle (SCROLLBAR, BORDER_WIDTH) + GuiGetStyle (SCROLLBAR, SCROLL_PADDING), arrowUpLeft.y + arrowUpLeft.height,
            ...
            slider = RAYGUI_CLITERAL (Rectangle){ bounds.x + GuiGetStyle (SCROLLBAR, BORDER_WIDTH) + GuiGetStyle (SCROLLBAR, SCROLL_SLIDER_PADDING),
        ...
        {
            arrowDownRight
                = RAYGUI_CLITERAL (Rectangle){ (float)bounds.x + bounds.width - spinnerSize - GuiGetStyle (SCROLLBAR, BORDER_WIDTH),
                                               (float)bounds.y + GuiGetStyle (SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
            scrollbar = RAYGUI_CLITERAL (Rectangle){
            ...
            slider     = RAYGUI_CLITERAL (
                Rectangle){ scrollbar.x + (int)(((float)(value - minValue) / valueRange) * (scrollbar.width - sliderSize)),
                                bounds.y + GuiGetStyle (SCROLLBAR, BORDER_WIDTH) + GuiGetStyle (SCROLLBAR, SCROLL_SLIDER_PADDING), (float)sliderSize,
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
        {
            ...
            if (guiControlExclusiveMode) // Allows to keep dragging outside of bounds
                {
                    if (GUI_BUTTON_DOWN && !CheckCollisionPointRec (mousePoint, arrowUpLeft) && !CheckCollisionPointRec (mousePoint, arrowDownRight))
                        {
                            if (CHECK_BOUNDS_ID (bounds, guiControlExclusiveRec))
                                {
                                    state = STATE_PRESSED;
