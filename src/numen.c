...
numen_add_skill (Numen* numen, Id skill_id)
{
	...
	for (i = 0; i < NUM_SKILLS; i++)
	{
		...
		{numen->skills[i] = skill_id;return OK;}
	}
...
{
	int i;
	if (!numen || skill_id < NO_SKILL) return NO_SKILL;

	for (i = 0; i < NUM_SKILLS; i++)
		if (numen->skills[i] == skill_id) return NO_SKILL;
	return NO_SKILL;
}

Skills_id numen_get_skill_by_index (Numen* numen, int skill_indx)
{
	if (!numen || skill_indx < NO_SKILL || skill_indx >= NUM_SKILLS) return NO_SKILL;
