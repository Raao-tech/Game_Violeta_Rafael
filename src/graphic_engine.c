...
graphic_engine_create (void)
{
	...
	/*Creamos el space graphic*/
	gp->spc                    = entity_ge_create ();
	if (!gp->spc)	{free (gp);		return NULL;}

	/*Creamos el objeto gráfico*/
	gp->obj = entity_ge_create ();
	if (!gp->obj){entity_ge_destroy (gp->spc); free (gp); return NULL;}

	/*Creamos los numens graficos*/
	gp->numens = entity_ge_create ();
	if (!gp->numens)
		{entity_ge_destroy (gp->obj);entity_ge_destroy (gp->spc);free (gp);return NULL;}
