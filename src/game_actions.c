...
game_actions_move (Game* game)
{
    if (!game || game_rule_move (game) == ERROR)
      ...
      player_set_show_message (game_get_player_at(game, PLAYER), TRUE);
		game_set_last_cmd_status (game, OK);
}
...
game_actions_walk (Game* game)
{
    ...
    if (!player)
	  ...
	  player_set_show_message (player, FALSE);
    ...
    if (types_position_is_valid ((void*)player, (Position (*) (void*))player_get_vision, 0, 0, WIDHT_MAP, HIGHT_MAP) == FALSE)
        {
            ...
            if (vision.pos_y < 0) player_set_vision_y (player, position.pos_y + SCALE);
            if (vision.pos_y > HIGHT_MAP) player_set_vision_y (player, position.pos_y - SCALE);
...
game_actions_take (Game* game)
{
    ...
    /* Si no habia ningún objeto activo, este pasa a ser el activo */
    if (player_get_active_object (player) == NO_ID) player_set_active_object (player, obj_id);
	  player_set_show_message (player, FALSE);
    game_set_last_cmd_status (game, OK);
}
...
game_actions_attack (Game* game)
{
    ...
    /* Aplicamos el skill a TODOS los numens validos en el radio:
     * enemigos corruptos Y capturables no corruptos (para poder debilitarlos). */
    for (i = 0; i < num_enemies; i++)
        {
            ...
            /* Solo saltar nuestros propios numens; el resto son objetivos validos */
            if (player_contains_numen (player, numen_get_id (enemy_num))) continue;
            if (numen_get_health (enemy_num) <= 0) continue;

            enemy_pos = numen_get_position (enemy_num);
            distance  = (int)sqrt (pow (active_pos.pos_x - enemy_pos.pos_x, 2) + pow (active_pos.pos_y - enemy_pos.pos_y, 2));
            ...
            /* Si el objetivo murió, procesamos su muerte */
            if (numen_get_health (enemy_num) <= 0) game_rules_death_numen (game, enemy_num);
                    ...
                    for (j = 0; j < num_enemies; j++)
                        {
                            ...
                            /* Splash afecta a cualquier numen que no sea nuestro */
                            if (player_contains_numen (player, numen_get_id (secondary))) continue;
                            ...
                            if (sec_dist <= radio_a)
                                {
                                    ...
                                    if (numen_get_health (secondary) <= 0) game_rules_death_numen (game, secondary);
                                }
                        }
                }
        }
...
game_actions_chat (Game* game)
{
    ...
    if (space_numens)
        {
            ...
            for (i = 0; i < space_get_n_numens (space); i++)
                {
                    char_id = set_get_id_at (space_numens, i);
                    ch      = game_get_numen_by_id (game, char_id);
                    if (!ch) continue;
                    char_pos = numen_get_position (ch);
                    if (char_pos.pos_x == ply_vision.pos_x && char_pos.pos_y == ply_vision.pos_y) {
                        found = TRUE;
                        break;
                    }
                }
    ...
    player_set_message (player, numen_get_gdesc (ch));
    player_set_show_message (player, TRUE);
    game_set_last_cmd_status (game, OK);
}
...
game_actions_inspect (Game* game)
{
    ...
    if (space_get_n_objects (space) > 0)
        {
            ...
            for (i = 0; i < space_get_n_objects (space); i++)
                {
                    char_id = space_get_object_id_at (space, i);
                    ch      = game_get_object_by_id (game, char_id);
                    if (!ch) continue;
                    char_pos = obj_get_position (ch);
                    if (char_pos.pos_x == ply_vision.pos_x && char_pos.pos_y == ply_vision.pos_y) {
                        found = TRUE;
                        break;
                    }
    ...
    player_set_message (player, obj_get_description (ch));
    player_set_show_message (player, TRUE);
    game_set_last_cmd_status (game, OK);
}
...
game_actions_save (Game* game)
{
    if (!game)
	  ...
	  player_set_show_message (game_get_player_at (game, PLAYER), FALSE);
...
game_actions_recruit (Game* game)
{
    ...
    /* Reset de posición: el numen ya no tiene coordenadas en el mapa */
    numen_set_position (best, NO_POS, NO_POS);

    /* Si el jugador no tenía numen activo, este pasa a serlo */
    if (player_get_active_numen (player) == NO_ID) player_set_active_numen (player, numen_get_id (best));
	  player_set_show_message (player, FALSE);
    game_set_last_cmd_status (game, OK);
}
    ...
    if (numen_char)
        {
            ...
            numen_set_following (numen, NO_ID);

            if (player_get_active_numen (player) == numen_id) player_set_active_numen (player, ga_player_cycle_inventory_numens (player));
        ...
        {
            /* MODO USER: suelta el numen activo en la casilla que mira el jugador */
            ply_vision = player_get_vision (player);
            if (types_position_is_valid (player, (Position (*) (void*))player_get_vision, 0, 0, WIDHT_MAP, HIGHT_MAP) == FALSE)
                {
                    game_set_last_cmd_status (game, ERROR_kick);
            ...
            if (player_delete_numen (player, numen_id) == ERROR)
                {
                    game_set_last_cmd_status (game, ERROR_kick);
                    return;
                }

            if (space_set_numen (space, numen_id, ply_vision) == ERROR)
                {
                    player_add_numen (player, numen_id);
                    game_set_last_cmd_status (game, ERROR_kick);
                    return;
                }

            numen_set_position (numen, ply_vision.pos_x, ply_vision.pos_y);
            numen_set_following (numen, NO_ID);
            player_set_active_numen (player, ga_player_cycle_inventory_numens (player));
...
static Status
_apply_attack_plus (Game* game, Object* obj)
{
    Numen* numen_active;
    Player* player;
    Id active_id;
...
static Status
_apply_attack_less (Game* game, Object* obj)
{
    Numen* numen_active;
    Player* player;
    Id active_id;
...
static Status
_apply_energy_plus (Game* game, Object* obj)
{
    Numen* numen_active;
    Player* player;
    Id active_id;
...
static Status
_apply_energy_less (Game* game, Object* obj)
{
    Numen* numen_active;
    Player* player;
    Id active_id;
...
static Status
_apply_multi_effect (Game* game, Object* obj)
{
    Numen* numen_active;
    Player* player;
    Id active_id;
