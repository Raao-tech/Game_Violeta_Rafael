                    ...
                    if (obj != NULL)
                        {
                            ...
                            if (space) { space_set_object (space, id, obj_get_position (obj)); }
                            else
                                {
    ...
    for (bucle = 0; bucle < check; bucle++)
        {
            ...
            fprintf (new_sfile, "#s:%ld|%s|%s|%s|", id, name, gdesc == NULL ? "" : gdesc, OST == NULL ? "" : OST);

            for (bucle = 0; bucle < HIGHT; bucle++)
                {
                    grid_line = space_get_grid_by_line (space, bucle);
                    fprintf (new_sfile, "|%d|", grid_line);
                }
            fprintf (new_sfile, "\n");
            free (name);
            free (gdesc);
