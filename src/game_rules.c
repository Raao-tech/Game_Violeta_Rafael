...
game_rule_walk_enemy (Game* game)
{
    ...
    if (active_id != NO_ID) active_numen = game_get_numen_by_id (game, active_id);
    ...
    for (i = 0; i < n_numens; i++)
        {
            ...
            /* === DECIDIR DIRECCION === */
            sees_active = (active_numen != NULL) && (_game_rule_numen_in_area (walker, active_numen, SCALE * 3) == TRUE);

            hp_now      = numen_get_health (walker);
            hp_max      = (numen_get_corrupt (walker) == TRUE) ? MAX_LIFE_CORRUPT : MAX_LIFE;
            hides       = (numen_get_corrupt (walker) == TRUE) && (hp_now > 0) && (hp_now <= hp_max * 30 / 100);

            /*Verificamos si el active está vivo, si el walker es corrupto y si el active está en el área de visión de walker*/
            if (active_numen != NULL && numen_get_corrupt (walker) == TRUE && numen_get_health (active_numen) > 0
                && _game_rules_numen_in_area (walker, active_numen, SCALE * 2) == TRUE)
                {
            ...
            switch (dir) /*Falta definir cuánto se mueve*/
                {
                    case N:
                        if (pos_now.pos_y - SCALE <= 0) pos_now.pos_y += SCALE;
                        pos_now.pos_y -= SCALE;
                        break;

                    case S:
                        if (pos_now.pos_y + SCALE >= HIGHT_MAP) pos_now.pos_y -= SCALE;
                        pos_now.pos_y += SCALE;
                        break;

                    case W:
                        if (pos_now.pos_x - SCALE <= 0) pos_now.pos_x += SCALE;
                        pos_now.pos_x -= SCALE;
                        break;

                    case E:
                        if (pos_now.pos_x + SCALE >= WIDHT_MAP) pos_now.pos_x -= SCALE;
                        pos_now.pos_x += SCALE;
                        break;
            ...
            if (cell_x < 0 || cell_x >= WIDHT || cell_y < 0 || cell_y >= HIGHT) continue;
...
game_rule_walk_active (Game* game)
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Dividiendo distance entre SCALE, se activa el movimiento aleatorio del numen. Este es un tipo de movimiento que actualmente está defectuoso, no
    // usar.
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
